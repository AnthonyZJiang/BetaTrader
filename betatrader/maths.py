...
def ma(values: np.array, period: int) -> np.array:
...
def emas(values: np.array, first_ema: int, period: int, k: int = None) -> np.array:
...
def sma(values: np.array, period: int, weight: int, first_sma: int) -> np.array:
    """Calculate Simple Moving Average"""
    n = len(values)
    smas = np.zeros(n)
    smas[0] = first_sma
...
def macd(fast_ema: np.array, slow_ema: np.array, first_dea: float) -> tuple[np.array, np.array, np.array]:
    diffs = fast_ema - slow_ema
    deas = emas(diffs, first_dea, 9)
    macds = (diffs - deas) * 2
    return diffs, deas, macds

def rsv(closing_prices: np.array, high_prices:np.array, low_prices:np.array, period:int) -> np.array:
...
def kdj(rsv9: np.array, k1: int = 50, d1: int = 50) -> np.array:
    """Calculate KDJ"""
    n = len(rsv9)
    ks = np.zeros(n)
    ds = np.zeros(n)
